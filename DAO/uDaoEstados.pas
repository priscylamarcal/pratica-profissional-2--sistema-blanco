...
type daoEstados = class( DAO )
  ...
  public
    ...
    function VerificaExiste(oEstado: Estados):Boolean;
...
begin
  ...
  mEstado.setUltAlt( aDM.QEstados.FieldByName('ULTALT').AsDateTime );
end;
...
//     end;
  ...
  begin
    ...
    try
      ...
      begin
         aDM.QEstados.Insert;
         aDM.QEstados.FieldByName('DATACAD').AsDateTime:= now
      end
      else
         aDM.QEstados.Edit;
      ...
      aDM.QEstados.FieldByName('ULTALT').AsDateTime:= now;
      aDM.QEstados.Post;
      aDM.Transacao.Commit;
    except
      aDM.Transacao.Rollback;
...
function daoEstados.ValidaExclusao(pObj: TObject): Boolean;
var
  mEstado : Estados;
begin
  mEstado:= Estados(pObj);
  Result := not(aDM.QCidades.Locate('CODESTADO', mEstado.getCodigo, []))
end;

function daoEstados.VerificaExiste(oEstado: Estados): Boolean;
begin
  if aDm.QEstados.Locate('ESTADO', oEstado.getEstado, []) then
