...
begin
  ...
  aCtrlEstados.setPosicao(Cidades( pObj ).getoEstado.GetCodigo);
...
procedure ctrlCidades.setPosicao(acodigo: integer);
begin
  aDaoCidades.setPosicao(aCodigo);
end;

function ctrlCidades.ValidaExclusao(pObj: TObject): Boolean;
begin
  result := aDaoCidades.ValidaExclusao(pObj);
end;

function ctrlCidades.VerificaExiste(aCidade: Cidades): Boolean;
begin
  result := aDaoCidades.VerificaExiste(aCidade);
end;
