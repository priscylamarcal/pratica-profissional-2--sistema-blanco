...
begin
  ...
  self.ComboBox_tipo_fornecedor.Enabled := False;
  ...
  self.edt_data_nasc.enabled := false;
...
begin
  // inherited;
  aConsultacidades.conhecaObj(aCtrlFornecedores.getCtrlCidades,
    oFornecedor.getaCidade);
...
begin
  inherited;
  aConsultacondicao.conhecaObj(aCtrlFornecedores.getCtrlCondicoes,
    oFornecedor.getaCondicao);
  ...
  self.edt_pesquisa_condicao_pagamento.Text :=
    oFornecedor.getaCondicao.getCondicao;
end;
...
//  self.edt_algo.Text := oFornecedor.getContatoAux2;
  ...
  self.edt_uf.Text := oFornecedor.getaCidade.getoEstado.getUF;
  if oFornecedor.getTipoForn = 'PESSOA FÍSICA' then
  begin
    self.ComboBox_tipo_fornecedor.ItemIndex := 0;
  end
  else
    self.ComboBox_tipo_fornecedor.ItemIndex := 1;
  tipoFornecedor;
...
begin
  ...
  self.ComboBox_tipo_fornecedor.Enabled:= True;
  ...
  self.edt_data_nasc.enabled := True;
...
//    oFornecedor.setContatoAux2(self.edt_algo.Text);
    ...
    oFornecedor.getaCondicao.setCondicao
      (self.edt_pesquisa_condicao_pagamento.Text);
    ...
    begin
      if (oFornecedor.getCodigo = 0) then
      begin
        if (aCtrlFornecedores.VerificaExiste(oFornecedor)) then
        // verifica se existe Fornecedor quando é incluir
        begin
          ShowMessage('Já existe um Fornecedor cadastrado com este CNPJ/CPF!');
      ...
      begin
        if not(CpfCnpjFornecedor = oFornecedor.getCnpjCpf) then
        // verifica se mudou o nome na alteracao
          if aCtrlFornecedores.VerificaExiste(oFornecedor) then
          // se mudou verifica se já existe Fornecedor com o nome
          begin
            aCtrlFornecedores.SetPosicao(oFornecedor.getCodigo);
            ShowMessage
              ('Já existe um Fornecedor cadastrado com este CNPJ/CPF!');
            edt_cpf_cnpj.SetFocus;
            Exit;
          end;
      end;
  ...
  case ComboBox_tipo_fornecedor.ItemIndex of
    0: // pessoa física
      begin
        ...
        lbl_data_fundacao_nascimento.Caption:= 'Data Nascimento';
      end;
    1: // pessoa jurídica
      begin
        ...
        lbl_data_fundacao_nascimento.Caption:= 'Data Fundação';
      end;
  end;
end;
...
begin
  ...
  begin
    MessageDlg('O campo Nome/Razão Social é obrigatório!', MtInformation,
      [MbOK], 0);
    edt_nome_razao_social.SetFocus;
    Exit;
  end;
  ...
  begin
    MessageDlg('O campo Apelido/Nome Fantasia é obrigatório!', MtInformation,
      [MbOK], 0);
  ...
  cpfcnpj := StringReplace(self.edt_cpf_cnpj.text, ' ', '', [rfReplaceAll]);

  if self.ComboBox_tipo_fornecedor.ItemIndex = 0 then
  begin
     if self.edt_data_nasc.date > now then
     begin
      MessageDlg('A data de nascimento não pode ser maior que a data atual!', MtInformation,
          [MbOK], 0);
      edt_data_nasc.SetFocus;
      Exit;
     end;

     if length(cpfcnpj) < 14 then
     begin
      MessageDlg('Complete corretamente o CPF!', MtInformation,
          [MbOK], 0);
      edt_cpf_cnpj.SetFocus;
      Exit;
     end;
  ...
  begin
    if self.edt_data_nasc.date > now then
     begin
      MessageDlg('A data de fundação não pode ser maior que a data atual!', MtInformation,
          [MbOK], 0);
      edt_data_nasc.SetFocus;
      Exit;
     end;

     if length(cpfcnpj) < 18 then
     begin
      MessageDlg('Complete corretamente o CNPJ!', MtInformation,
          [MbOK], 0);
      edt_cpf_cnpj.SetFocus;
      Exit;
     end;
