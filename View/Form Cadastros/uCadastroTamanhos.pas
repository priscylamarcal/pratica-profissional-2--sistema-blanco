...
begin
  ...
  NomeTamanho := oTamanho.getSiglaTamanho;
end;
...
//    oTamanho.setUltAlt(Date);
    ...
    begin
      if (oTamanho.getCodigo = 0) then
      begin
        if (aCtrlTamanhos.VerificaExiste(oTamanho.getSiglaTamanho)) then
        // verifica se existe tamanho quando é incluir
        begin
          ShowMessage('Já existe um tamanho ' + oTamanho.getSiglaTamanho +
            ' cadastrado!');
          edt_sigla_tamanho.SetFocus;
          Exit;
        end;
      ...
      begin
        if not(NomeTamanho = oTamanho.getSiglaTamanho) then
          if (aCtrlTamanhos.VerificaExiste(oTamanho.getSiglaTamanho)) then
          // se mudou verifica se já existe tamanho com o nome
          begin
            aCtrlTamanhos.SetPosicao(oTamanho.getCodigo);
            ShowMessage('Já existe um tamanho ' + oTamanho.getSiglaTamanho +
              ' cadastrado!');
            edt_sigla_tamanho.SetFocus;
            Exit;
    ...
    else if aCtrlTamanhos.ValidaExclusao(oTamanho.clone) then
      aCtrlTamanhos.excluir(oTamanho.clone)
    else
      ShowMessage
        ('O tamanho está vinculado a uma ou mais roupas, não pode ser excluído!');
...
begin
  ...
  begin
    MessageDlg('O campo Sigla Tamanho é obrigatório!', MtInformation,
      [MbOK], 0);
    edt_sigla_tamanho.SetFocus;
    Exit;
  end;
