...
//    aCidade.setUltAlt(Date);
    ...
    begin
      if (aCidade.getCodigo = 0) then
      begin
        if (aCtrlCidades.VerificaExiste(aCidade)) then
        // verifica se existe Cidade quando é incluir
        begin
          ShowMessage('Já existe uma Cidade' + aCidade.getCidade +
            ' cadastrada nesse Estado ' + aCidade.getoEstado.getEstado + '!');
          edt_cidade.SetFocus;
          Exit;
        end;
      end
      ...
      begin
        if not(NomeCidade = aCidade.getCidade) then
        // verifica se mudou o nome na alteracao
          if aCtrlCidades.VerificaExiste(aCidade) then
          // se mudou verifica se já existe Cidade com o nome
          begin
            aCtrlCidades.SetPosicao(aCidade.getCodigo);
            ShowMessage('Já existe uma Cidade ' + aCidade.getCidade +
              ' cadastrada nesse Estado ' + aCidade.getoEstado.getEstado + '!');
            edt_cidade.SetFocus;
            Exit;
    ...
    else if aCtrlCidades.ValidaExclusao(aCidade.clone) then
      aCtrlCidades.excluir(aCidade.clone)
    else
      ShowMessage
        ('A cidade está sendo usada por outro registro, não pode ser excluída!');
