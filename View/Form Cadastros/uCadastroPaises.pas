...
begin
  ...
  if oPais.getUltAlt = 0 then
    edt_data_ult_alt.Text := ''
  else
...
//    end;
    ...
    begin
      if (oPais.getCodigo = 0) then
      begin
        if (aCtrlPais.VerificaExiste(oPais.getPais)) then
    ...
    begin
      if aCtrlPais.ValidaExclusao(oPais.clone) then
        ...
        ShowMessage
          ('O País está cadastrado em um estado, não pode ser excluído!');
    end;
