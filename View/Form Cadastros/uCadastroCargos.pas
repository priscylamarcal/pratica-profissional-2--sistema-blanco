...
begin
  // inherited;
  aConsultaDepartamentos.conhecaObj(aCtrlCargos.getCtrlDepartamentos,
    oCargo.getoDepartamento);
  ...
  self.edt_pesquisar_departamento_cargo.Text :=
    oCargo.getoDepartamento.getDepartamento;
end;
...
begin
  ...
  begin
    ...
    oCargo.getoDepartamento.setDepartamento
      (edt_pesquisar_departamento_cargo.Text);
...
//    oCargo.setUltAlt(Date);
    ...
    begin
      if (oCargo.getCodigo = 0) then
      begin
        if (aCtrlCargos.VerificaExiste(oCargo)) then
        // verifica se existe pais quando é incluir
        begin
          ShowMessage('Já existe um Cargo ' + oCargo.getCargo +
            ' cadastrado no departamento ' +
            oCargo.getoDepartamento.getDepartamento + ' !');
          edt_cargo.SetFocus;
          Exit;
        end;
      ...
      begin
        if not(NomeCargo = oCargo.getCargo) then
          if (aCtrlCargos.VerificaExiste(oCargo)) then
          // se mudou verifica se já existe pais com o nome
          begin
            aCtrlCargos.SetPosicao(oCargo.getCodigo);
            ShowMessage('Já existe um Cargo ' + oCargo.getCargo +
              ' cadastrado no departamento ' +
              oCargo.getoDepartamento.getDepartamento + ' !');
            edt_cargo.SetFocus;
      ...
      if aCtrlCargos.ValidaExclusao(oCargo.clone) then
        aCtrlCargos.excluir(oCargo.clone)
      else
        ShowMessage
          ('O Cargo está vinculado a um ou mais Funcionários, não pode ser excluído!');
