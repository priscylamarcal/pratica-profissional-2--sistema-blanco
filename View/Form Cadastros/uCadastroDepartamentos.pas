...
begin
  ...
  self.edt_data_ult_alt.Text := DateToStr(oDepartamento.getUltAlt);
...
//    oDepartamento.setUltAlt(Date);
    ...
    begin
      if (oDepartamento.getCodigo = 0) then
      begin
        if (aCtrlDepartamentos.VerificaExiste(oDepartamento.getDepartamento))
        then // verifica se existe pais quando é incluir
        begin
          ShowMessage('Já existe um Departamento ' +
            oDepartamento.getDepartamento + ' cadastrado!');
      ...
      begin
        if not(NomeDepartamento = oDepartamento.getDepartamento) then
          if (aCtrlDepartamentos.VerificaExiste(oDepartamento.getDepartamento))
          then // se mudou verifica se já existe pais com o nome
          begin
    ...
    else if aCtrlDepartamentos.ValidaExclusao(oDepartamento.clone) then
      aCtrlDepartamentos.excluir(oDepartamento.clone)
    else
      ShowMessage
        ('O Departamento está vinculado a um ou mais Cargos, não pode ser excluído!');
